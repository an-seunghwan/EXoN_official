...
def _list_to_tf_dataset(dataset, args):
    ...
    return tf.data.Dataset.from_generator(
        ...
        else {"image": tf.uint8, "label": tf.int64},
...
def split_dataset(dataset, num_labeled, num_validations, num_classes, args):
    ...
    for example in tqdm(iter(dataset), desc="split_dataset"):
        ...
        if counter[label] <= (num_validations / num_classes):
            validation.append({"image": example["image"], "label": example["label"]})
        elif counter[label] <= (
            num_validations / num_classes + num_labeled / num_classes
        ):
            labeled.append({"image": example["image"], "label": example["label"]})
        unlabeled.append(
            {
                "image": example["image"],
...
def fetch_dataset(args, log_path):
    ...
    if any(
        [
            not os.path.exists(f"{dataset_path}/{split}.tfrecord")
            for split in ["trainL", "trainU", "validation", "test"]
        ]
    ):
        ...
        for name, dataset in [
            ...
            writer = tf.io.TFRecordWriter(
                f"{dataset_path}/{name}.tfrecord".encode("utf-8")
            )
            for x in tfds.as_numpy(dataset):
                example = serialize_example(x, num_classes, args)
                writer.write(example)

    trainL = tf.data.TFRecordDataset(
        f"{dataset_path}/trainL.tfrecord".encode("utf-8")
    ).map(deserialize_example)
    trainU = tf.data.TFRecordDataset(
        f"{dataset_path}/trainU.tfrecord".encode("utf-8")
    ).map(deserialize_example)
    validation = tf.data.TFRecordDataset(
        f"{dataset_path}/validation.tfrecord".encode("utf-8")
    ).map(deserialize_example)
    test = tf.data.TFRecordDataset(f"{dataset_path}/test.tfrecord".encode("utf-8")).map(
        deserialize_example
    )
