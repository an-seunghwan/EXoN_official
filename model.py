...
class Encoder(K.models.Model):
    def __init__(self, latent_dim, num_classes, name="Encoder", **kwargs):
        ...
        self.net = K.Sequential(
            [
                layers.Conv2D(
                    filters=self.nChannels[0], kernel_size=5, strides=2, padding="same"
                ),  # 16x16
                layers.BatchNormalization(),
                layers.ReLU(),
                layers.Conv2D(
                    filters=self.nChannels[1], kernel_size=4, strides=2, padding="same"
                ),  # 8x8
                layers.BatchNormalization(),
                layers.ReLU(),
                layers.Conv2D(
                    filters=self.nChannels[2], kernel_size=4, strides=2, padding="same"
                ),  # 4x4
                layers.BatchNormalization(),
                layers.ReLU(),
                layers.Conv2D(
                    filters=self.nChannels[3], kernel_size=4, strides=1, padding="same"
                ),
                layers.BatchNormalization(),
                layers.ReLU(),
                layers.Flatten(),
        ...
        self.mean_layer = [
            layers.Dense(latent_dim, activation="linear") for _ in range(num_classes)
        ]
        self.logvar_layer = [
            layers.Dense(latent_dim, activation="linear") for _ in range(num_classes)
        ]
    ...
    def call(self, x, training=True):
        ...
        mean = layers.Concatenate(axis=1)(
            [d(h)[:, tf.newaxis, :] for d in self.mean_layer]
        )
        logvar = layers.Concatenate(axis=1)(
            [d(h)[:, tf.newaxis, :] for d in self.logvar_layer]
        )
        return mean, logvar
...
class Classifier(K.models.Model):
    def __init__(self, num_classes, dropratio=0.1, name="Classifier", **kwargs):
    ...
    def __init__(self, channel, activation, name="Decoder", **kwargs):
        ...
        self.net = K.Sequential(
            [
                ...
                layers.ReLU(),
                layers.Conv2DTranspose(
                    filters=self.nChannels[1], kernel_size=5, strides=2, padding="same"
                ),
                layers.BatchNormalization(),
                layers.ReLU(),
                layers.Conv2DTranspose(
                    filters=self.nChannels[2], kernel_size=5, strides=2, padding="same"
                ),
                layers.BatchNormalization(),
                layers.ReLU(),
                layers.Conv2DTranspose(
                    filters=self.nChannels[3], kernel_size=5, strides=1, padding="same"
                ),
                layers.BatchNormalization(),
                layers.ReLU(),
                layers.Conv2D(
...
class MixtureVAE(K.models.Model):
    ...
    def gumbel_max_sample(self, probs):
        ...
        if self.hard:
            y_hard = tf.cast(
                tf.equal(y, tf.math.reduce_max(y, 1, keepdims=True)), y.dtype
            )
    ...
    def encode(self, x, training=True):
        ...
        z = mean + tf.math.exp(logvar / 2.0) * epsilon
