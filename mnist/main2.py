...
def get_args():
    ...
    parser.add_argument('--rampup_epoch',default=25, type=int, 
                        help='the max epoch to adjust learning rate and unsupervised weight')
    parser.add_argument('--rampdown_epoch',default=5, type=int, 
...
def main():
    ...
    for epoch in range(args['start_epoch'], args['epochs']):
        ...
        # ramp-up
        optimizer.lr = weight_schedule(epoch, args['rampup_epoch'], args['lr'])
        # ramp-down
        if epoch >= args['epochs'] - args['rampdown_epoch']:
            optimizer.lr = args['lr'] * tf.math.exp(-12.5 * (1. - (args['epochs'] - epoch) / args['rampdown_epoch']) ** 2)
...
def train(datasetL, datasetU, model, buffer_model, optimizer, epoch, args, beta, prior_means, sigma, num_classes, total_length, test_accuracy_print):
    ...
    unlabel_lambda1 = weight_schedule(epoch, args['rampup_epoch'], args['lambda1'] * (args['labeled_examples'] / total_length))
    ...
    for batch_num in progress_bar:
        ...
        with tf.GradientTape(persistent=True) as tape:    
            ...
            probU_noise = model.classify(imageU, noise=True)
            # unlabel_recon = tf.reduce_sum(tf.reduce_sum(tf.math.square(probU - probU_noise), axis=-1))
            # CE
            unlabel_recon = - tf.reduce_sum(tf.reduce_sum(probU * tf.math.log(tf.clip_by_value(probU_noise, 1e-10, 1.0)), axis=-1))
            # JSD
