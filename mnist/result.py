...
classification_error = (
    np.sum((tf.argmax(labels, axis=1) - tf.argmax(probs, axis=1) != 0).numpy())
    / total_length
)
...
kl1 = tf.reduce_mean(
    tf.reduce_sum(
        probs
        * (
            tf.math.log(tf.clip_by_value(probs, 1e-10, 1.0))
            + tf.math.log(tf.cast(num_classes, tf.float32))
        ),
        axis=1,
    )
)
    ...
    tf.reduce_sum(
        tf.multiply(
            ...
            tf.reduce_sum(
                0.5
                * (
                    tf.math.pow(means - prior_means, 2) / sigma
                    - 1
                    + tf.math.log(sigma)
...
plt.scatter(
    zmat[:, 0],
    zmat[:, 1],
    c=tf.argmax(labels, axis=1).numpy(),
    s=10,
    cmap=plt.cm.Reds,
    alpha=1,
)
plt.savefig(
    "./{}/latent.png".format(model_path), dpi=200, bbox_inches="tight", pad_inches=0.1
)
plt.show()
plt.close()
#%%
...
for i in range(len(grid)):
    ...
    plt.tight_layout()
plt.savefig(
    "./{}/reconstruction.png".format(model_path),
    dpi=200,
    bbox_inches="tight",
    pad_inches=0.1,
)
...
plt.savefig(
    "./{}/conditional_prob.png".format(model_path),
    dpi=200,
    bbox_inches="tight",
    pad_inches=0.1,
)
...
for i in range(num_classes):
    samples.extend(
        np.random.multivariate_normal(
            mean=prior_means.numpy()[0][i, :2],
            cov=np.array([[sigma.numpy(), 0], [0, sigma.numpy()]]),
            size=1000,
        )
    )
    color.extend([i] * 1000)
...
plt.plot(
    (z_inter[0][0], z_inter[1][0]),
    (z_inter[0][1], z_inter[1][1]),
    color="black",
    linewidth=2,
    linestyle="--",
)
...
plt.savefig(
    "./{}/interpolation_path.png".format(model_path),
    dpi=200,
    bbox_inches="tight",
    pad_inches=0.1,
)
...
for i in tqdm.tqdm(range(len(grid_output))):
    s = tf.image.ssim(
        tf.reshape(grid_output[i, :], (28, 28, 1)),
        tf.reshape(grid_output, (len(grid_output), 28, 28, 1)),
        max_val=1.0,
        filter_size=11,
        filter_sigma=1.5,
        k1=0.01,
...
for i in range(10):
    ...
    plt.axis("off")
plt.savefig(
    "./{}/interpolation_path_recon.png".format(model_path),
    dpi=200,
    bbox_inches="tight",
    pad_inches=0.1,
)
...
for l in betas:
    img = [
        Image.open("./logs/mnist_100/beta_{}/interpolation_path.png".format(l)),
    ...
    plt.tight_layout()
    plt.savefig(
        "./logs/mnist_100/beta_{}/interpolation_path_and_recon.png".format(l),
        dpi=200,
        bbox_inches="tight",
        pad_inches=0.1,
    )
...
for l in betas:
    img.append(
        [
            Image.open("./logs/mnist_100/beta_{}/latent.png".format(l)),
...
plt.savefig(
    "./logs/mnist_100/path_latent_recon.png",
    dpi=200,
    bbox_inches="tight",
    pad_inches=0.1,
)
...
pd.DataFrame.from_dict(errors, orient="index").rename(columns={0: "test error"}).to_csv(
    log_path + "/test_error_path.csv"
)
pd.DataFrame.from_dict(kls, orient="index").rename(columns={0: "KL"}).to_csv(
    log_path + "/kl_path.csv"
)
pd.DataFrame.from_dict(ssims, orient="index").rename(
    columns={0: "negative SSIM"}
).to_csv(log_path + "/negative_ssim_path.csv")
#%%
