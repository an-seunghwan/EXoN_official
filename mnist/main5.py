...
"""
***check list!***
- perfectly white background -> MSE
...
def get_args():
    ...
    parser.add_argument(
        "--beta_trainable", default=False, type=bool, help="trainable beta"
    )

    """VAE parameters"""
    parser.add_argument(
        "--latent_dim",
        "--latent_dim_continuous",
        default=2,
        type=int,
    ...
    """Prior design"""
    parser.add_argument(
        "--sigma", default=4, type=float, help="variance of prior mixture component"
    )
    ...
    parser.add_argument(
        "--lambda1",
        default=6000,
        type=int,  # labeled dataset ratio?
...
def main():
    ...
    model = MixtureVAE(args, num_classes, latent_dim=args["latent_dim"])
    model.build(input_shape=(None, 28, 28, 1))
    model.summary()

    buffer_model = MixtureVAE(args, num_classes, latent_dim=args["latent_dim"])
    ...
    """prior design"""
    r = 2.0 * np.sqrt(args["sigma"]) / np.sin(np.pi / 10.0)
    prior_means = np.array(
        [
            [r * np.cos(np.pi / 10), r * np.sin(np.pi / 10)],
    ...
    for epoch in range(args["start_epoch"], args["epochs"]):
        ...
        if epoch >= args["rampdown_epoch"]:
            optimizer_classifier.lr = args["lr"] * tf.math.exp(
                -5 * (1.0 - (args["epochs"] - epoch) / args["epochs"]) ** 2
            )
...
def train(
    ...
    shuffle_and_batchL = (
        lambda dataset: dataset.shuffle(buffer_size=int(1e3))
        .batch(batch_size=32, drop_remainder=False)
        .prefetch(autotune)
    )
    shuffle_and_batchU = (
        lambda dataset: dataset.shuffle(buffer_size=int(1e6))
        .batch(batch_size=args["batch_size"] - 32, drop_remainder=True)
        .prefetch(autotune)
    )
    ...
    for batch_num in progress_bar:
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            if args["entropy_loss"]:
                ...
                entropyU = -tf.reduce_sum(
                    tf.reduce_sum(
                        tf.multiply(
                            probU_aug,
                            tf.math.log(tf.clip_by_value(probU_aug, 1e-10, 1.0)),
                        ),
                        axis=-1,
                    )
                )
                ...
                loss = (
                    elbo
                    + lambda1 * (cce + mixup_yL)
                    + unlabel_lambda1 * (mixup_yU + entropyU)
                )
            else:
                entropyU = 0.0
        ...
        weight_decay_decoupled(
            ...
            decay_rate=args["wd"] * optimizer_classifier.lr,
        )
...
def update_beta(model, datasetU, args, total_length):
    ...
    beta = beta / 60000.0 + tf.cast(args["lambda2"], tf.float32)
    return beta
