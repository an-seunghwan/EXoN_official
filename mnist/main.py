...
def get_args():
    ...
    parser.add_argument('--lambda1',default=1, type=int, 
...
def main():
    ...
    for epoch in range(args['start_epoch'], args['epochs']):
        ...
        if epoch % args['reconstruct_freq'] == 0:
            loss, recon_loss, info_loss, nf_z_loss, nf_c_loss, mixup_z_loss, mixup_xhat_loss, mixup_y_loss, accuracy, sample_recon = train(datasetL, datasetU, model, buffer_model, optimizer, epoch, args, prior_means, sigma_vector, num_classes, total_length, test_accuracy_print)
        else:
            loss, recon_loss, info_loss, nf_z_loss, nf_c_loss, mixup_z_loss, mixup_xhat_loss, mixup_y_loss, accuracy = train(datasetL, datasetU, model, buffer_model, optimizer, epoch, args, prior_means, sigma_vector, num_classes, total_length, test_accuracy_print)
...
def train(datasetL, datasetU, model, buffer_model, optimizer, epoch, args, prior_means, sigma_vector, num_classes, total_length, test_accuracy_print):
    ...
    for batch_num in progress_bar:
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            recon_lossU, kl_lossU = ELBO_criterion(probU, xhatU, imageU, meanU, logvarU, 
            ...
            cce = - tf.reduce_mean(tf.reduce_sum(tf.multiply(labelL, tf.math.log(probL + 1e-8)), axis=-1))
            ...
            lossU = (recon_lambda * recon_lossU + kl_lossU) + (mixup_lambda_y * mixup_yU)
