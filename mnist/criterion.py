...
#%%
def ELBO_criterion(prob, xhat, x, mean, logvar, beta, prior_means, sigma, num_classes, args):
    # reconstruction error
    if args['bce']:
        error = tf.reduce_mean(- tf.reduce_sum(x * tf.math.log(tf.clip_by_value(xhat, 1e-10, 1.)) + 
                                            (1. - x) * tf.math.log(1. - tf.clip_by_value(xhat, 1e-10, 1.)), axis=[1, 2, 3]) / beta)
    else:
        error = tf.reduce_mean(tf.reduce_sum(tf.math.square(x - xhat), axis=[1, 2, 3]) / (2 * beta))
