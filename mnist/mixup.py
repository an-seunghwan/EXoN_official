...
def augment(image):
    ...
    rotatedImg = [
        scipy.ndimage.interpolation.rotate(im, rotation[i], axes=(0, 1), mode="nearest")
        for i, im in enumerate(npimage)
    ]
    ...
    paddedImg = [
        np.pad(im, ((4, 4), (4, 4), (0, 0)), "constant", constant_values=-1)
        for im in rotatedImgCentered
    ]
