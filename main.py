...
def get_args():
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="reconstruct frequency (default: 10)",
    )
    parser.add_argument(
        "--labeled_examples",
        type=int,
    ...
    parser.add_argument(
        "--augment", default=True, type=bool, help="apply augmentation to image"
    )

    """Deep VAE Model Parameters"""
    parser.add_argument(
        "--drop_rate", default=0, type=float, help="drop rate for the network"
    )
    parser.add_argument(
        "--bce_reconstruction", default=False, type=bool, help="Do BCE Reconstruction"
    )
    ...
    """Prior design"""
    parser.add_argument(
        "--sigma1", default=0.1, type=float, help="variance of prior mixture component"
    )
    parser.add_argument(
        "--sigma2", default=1, type=float, help="variance of prior mixture component"
    )
    parser.add_argument(
        "--dist",
        default=1,
    ...
    parser.add_argument(
        "--beta", default=0.01, type=int, help="value of observation noise"
    )
    parser.add_argument(
        "--rampup_epoch",
        default=50,
    ...
    parser.add_argument(
        "--adjust_lr",
        default=[250, 350, 450],
        type=arg_as_list,  # classifier optimizer scheduling
        help="The milestone list for adjust learning rate",
    )
    parser.add_argument("--lr_gamma", default=0.5, type=float)
    parser.add_argument("--weight_decay", default=5e-4, type=float)
    parser.add_argument(
        "--epsilon", default=0.1, type=float, help="beta distribution parameter"
    )
...
def main():
    ...
    for epoch in range(args["start_epoch"], args["epochs"]):
        ...
        if epoch == 0:
            ...
            if epoch >= args["adjust_lr"][-1]:
                lr_ = args["learning_rate"] * (
                    args["lr_gamma"] ** len(args["adjust_lr"])
                )
                optimizer_classifier.lr = lr_ * tf.math.exp(
                    -5.0
                    * (
                        1.0
                        - (args["epochs"] - epoch)
                        / (args["epochs"] - args["adjust_lr"][-1])
                    )
                    ** 2
                )
            else:
        ...
        with train_writer.as_default():
            ...
            tf.summary.scalar(
                "unlabel_mixup_loss", unlabel_mixup_loss.result(), step=epoch
            )
    ...
    model.save_weights(
        model_path + "/model_{}.h5".format(current_time), save_format="h5"
    )
...
def train(
    ...
    shuffle_and_batchL = (
        lambda dataset: dataset.shuffle(buffer_size=int(1e5))
        .batch(batch_size=args["labeled_batch_size"], drop_remainder=False)
        .prefetch(autotune)
    )
    shuffle_and_batchU = (
        lambda dataset: dataset.shuffle(buffer_size=int(1e6))
        .batch(
            batch_size=args["batch_size"] - args["labeled_batch_size"],
            drop_remainder=True,
        )
        .prefetch(autotune)
    )
    ...
    for batch_num in progress_bar:
        ...
        """mix-up weight"""
        mix_weight = [
            tf.constant(np.random.beta(args["epsilon"], args["epsilon"])),  # labeled
            tf.constant(np.random.beta(2.0, 2.0)),
        ]  # unlabeled
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            cce = -tf.reduce_sum(
                tf.reduce_sum(
                    tf.multiply(
                        labelL, tf.math.log(tf.clip_by_value(probL_aug, 1e-10, 1.0))
                    ),
                    axis=-1,
                )
            )
            ...
            with tape.stop_recording():
                image_mixL, label_shuffleL = non_smooth_mixup(
                    imageL_aug, labelL, mix_weight[0]
                )
                # classifier output of right before epoch
                pseudo_labelU = buffer_model.classify(imageU_aug)
                image_mixU, pseudo_label_shuffleU = non_smooth_mixup(
                    imageU_aug, pseudo_labelU, mix_weight[1]
                )
            # labeled
            prob_mixL = model.classify(image_mixL)
            mixup_yL = -tf.reduce_sum(
                mix_weight[0]
                * tf.reduce_sum(
                    label_shuffleL
                    * tf.math.log(tf.clip_by_value(prob_mixL, 1e-10, 1.0)),
                    axis=-1,
                )
            )
            mixup_yL += -tf.reduce_sum(
                (1.0 - mix_weight[0])
                * tf.reduce_sum(
                    labelL * tf.math.log(tf.clip_by_value(prob_mixL, 1e-10, 1.0)),
                    axis=-1,
                )
            )
            # unlabeled
            prob_mixU = model.classify(image_mixU)
            mixup_yU = -tf.reduce_sum(
                mix_weight[1]
                * tf.reduce_sum(
                    pseudo_label_shuffleU
                    * tf.math.log(tf.clip_by_value(prob_mixU, 1e-10, 1.0)),
                    axis=-1,
                )
            )
            mixup_yU += -tf.reduce_sum(
        ...
        # encoder and decoder
        grads = tape.gradient(
            loss, model.decoder.trainable_variables + model.encoder.trainable_variables
        )
        optimizer.apply_gradients(
            zip(
                grads,
                model.decoder.trainable_variables + model.encoder.trainable_variables,
            )
        )
        # classifier
        grads = tape.gradient(loss, model.classifier.trainable_variables)
        optimizer_classifier.apply_gradients(
            zip(grads, model.classifier.trainable_variables)
        )
        """decoupled weight decay"""
        weight_decay_decoupled(
            model.classifier,
            buffer_model.classifier,
            decay_rate=args["weight_decay"] * optimizer_classifier.lr,
        )
    ...
    if epoch % args["reconstruct_freq"] == 0:
        sample_recon = generate_and_save_images1(
            model, imageU[0][tf.newaxis, ...], num_classes
        )
        generate_and_save_images2(
            ...
            num_classes,
            epoch,
            f'logs/{args["dataset"]}_{args["labeled_examples"]}/{current_time}',
        )
        return (
            loss_avg,
            recon_loss_avg,
            kl1_loss_avg,
            kl2_loss_avg,
            label_mixup_loss_avg,
            unlabel_mixup_loss_avg,
            accuracy,
            sample_recon,
        )
    else:
...
#%%
def validate(
    dataset, model, epoch, args, beta, prior_means, sigma_vector, num_classes, split
):
    ...
    for image, label in dataset:
        ...
        cce = -tf.reduce_sum(
            tf.reduce_sum(
                tf.multiply(label, tf.math.log(tf.clip_by_value(prob, 1e-10, 1.0))),
                axis=-1,
            )
        )
        ...
        accuracy(
            tf.argmax(prob, axis=1, output_type=tf.int32),
            tf.argmax(label, axis=1, output_type=tf.int32),
        )
    print(
        f"Epoch {epoch:04d}: {split} ELBO Loss: {elbo_loss_avg.result():.4f}, Recon: {recon_loss_avg.result():.4f}, KL1: {kl1_loss_avg.result():.4f}, KL2: {kl2_loss_avg.result():.4f}, Accuracy: {accuracy.result():.3%}"
    )
...
def weight_schedule(epoch, epochs, weight_max):
    return weight_max * tf.math.exp(-5.0 * (1.0 - min(1.0, epoch / epochs)) ** 2)
